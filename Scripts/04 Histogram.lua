local function gen_vertices(player, width, height, Steps, desaturation)
	...
	PeakNPS = InfinitesimalCache.Streams[pn].PeakNPS
	NPSperMeasure = InfinitesimalCache.Streams[pn].NPSperMeasure 
...
function NPS_Histogram_Static_Course(player, width, height, desaturation)
	...
	for te in ivalues(trail:GetTrailEntries()) do
		local w = (te:GetSong():GetLastSecond() / GAMESTATE:GetPlayerState(pn):GetPlayerOptions("ModsLevel_Song"):MusicRate() / totaltime) * width
