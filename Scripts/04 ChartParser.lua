-- Initialize cache for the theme
if not InfinitesimalCache then InfinitesimalCache = {} end
if not InfinitesimalCache.Streams then
	InfinitesimalCache.Streams = {
		P1 = {},
		P2 = {}
	}
end
...
local GetMeasureInfo = function(Steps, chartString)
	...
	for columnTime in ivalues(columnTimes) do
		...
		if duration >= 1.5 or prevTime == 0 then
...
local MaybeCopyFromOppositePlayer = function(pn, filename, stepsType, difficulty, description)
	...
	if (InfinitesimalCache.Streams[opposite_player].Filename == filename and
			InfinitesimalCache.Streams[opposite_player].StepsType == stepsType and
			InfinitesimalCache.Streams[opposite_player].Difficulty == difficulty and
			InfinitesimalCache.Streams[opposite_player].Description == description) then
		-- If so then just copy everything over.
		InfinitesimalCache.Streams[pn].NotesPerMeasure = InfinitesimalCache.Streams[opposite_player].NotesPerMeasure
		InfinitesimalCache.Streams[pn].EquallySpacedPerMeasure = InfinitesimalCache.Streams[opposite_player].EquallySpacedPerMeasure
		InfinitesimalCache.Streams[pn].PeakNPS = InfinitesimalCache.Streams[opposite_player].PeakNPS
		InfinitesimalCache.Streams[pn].NPSperMeasure = InfinitesimalCache.Streams[opposite_player].NPSperMeasure
		InfinitesimalCache.Streams[pn].ColumnCues = InfinitesimalCache.Streams[opposite_player].ColumnCues
		InfinitesimalCache.Streams[pn].Hash = InfinitesimalCache.Streams[opposite_player].Hash

		InfinitesimalCache.Streams[pn].Crossovers = InfinitesimalCache.Streams[opposite_player].Crossovers
		InfinitesimalCache.Streams[pn].Footswitches = InfinitesimalCache.Streams[opposite_player].Footswitches
		InfinitesimalCache.Streams[pn].Sideswitches = InfinitesimalCache.Streams[opposite_player].Sideswitches
		InfinitesimalCache.Streams[pn].Jacks = InfinitesimalCache.Streams[opposite_player].Jacks
		InfinitesimalCache.Streams[pn].Brackets = InfinitesimalCache.Streams[opposite_player].Brackets

		InfinitesimalCache.Streams[pn].Filename = InfinitesimalCache.Streams[opposite_player].Filename
		InfinitesimalCache.Streams[pn].StepsType = InfinitesimalCache.Streams[opposite_player].StepsType
		InfinitesimalCache.Streams[pn].Difficulty = InfinitesimalCache.Streams[opposite_player].Difficulty
		InfinitesimalCache.Streams[pn].Description = InfinitesimalCache.Streams[opposite_player].Description
...
ParseChartInfo = function(steps, pn)
	...
	-- Only parse the file if it's not what's already stored in Cache.
	if (InfinitesimalCache.Streams[pn].Filename ~= filename or
			InfinitesimalCache.Streams[pn].StepsType ~= stepsType or
			InfinitesimalCache.Streams[pn].Difficulty ~= difficulty or
			InfinitesimalCache.Streams[pn].Description ~= description) then
		...
		if simfileString then
			...
			if chartString ~= nil and BPMs ~= nil then
				...
				InfinitesimalCache.Streams[pn].NotesPerMeasure = NotesPerMeasure
				InfinitesimalCache.Streams[pn].EquallySpacedPerMeasure = EquallySpacedPerMeasure
				InfinitesimalCache.Streams[pn].PeakNPS = PeakNPS
				InfinitesimalCache.Streams[pn].NPSperMeasure = NPSperMeasure
				...
				InfinitesimalCache.Streams[pn].Difficulty = difficulty
				InfinitesimalCache.Streams[pn].Description = description
		...
		if not parsed then
			InfinitesimalCache.Streams[pn].NotesPerMeasure = {}
			InfinitesimalCache.Streams[pn].EquallySpacedPerMeasure = {}
			InfinitesimalCache.Streams[pn].PeakNPS = 0
			InfinitesimalCache.Streams[pn].NPSperMeasure = {}
			InfinitesimalCache.Streams[pn].Hash = ''
