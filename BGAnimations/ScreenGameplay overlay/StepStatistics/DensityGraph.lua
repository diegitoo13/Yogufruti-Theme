...
local IsUltraWide = (SCREEN_WIDTH / SCREEN_HEIGHT > 21/9)
local NoteFieldIsCentered = Center1Player() or GAMESTATE:GetIsFieldCentered(player)

local BothUsingStepStats = false
local FilterAlpha = 1.0
...
local histogram_amv = Scrolling_NPS_Histogram(player, width, height)..{
	...
	SizeCommand=function(self)
		-- Removed BothUsingStepStats vertical scaling to simplify
	end
}
...
local text = LoadFont("Common Normal")..{
	...
	PeakNPSUpdatedMessageCommand=function(self)
		local my_peak = InfinitesimalCache.Streams[pn].PeakNPS
		...
		self:settext( ("%s: %g"):format("Peak NPS", math.floor(my_peak * GAMESTATE:GetPlayerState(player):GetPlayerOptions("ModsLevel_Song"):MusicRate() * 100) / 100) )
	end,
}
		...
		UpdateCommand=function(self)
			if GAMESTATE:GetCurMusicSeconds() > 0 then
				...
				if LifeMeter then
					...
					y = 0 -- Fallback to max life if meter not found
				end
